...
import { qzlUrl, qzlOss } from '@config';
...
export default class app extends Component {
  ...
  // 获取活动列表数据
  getData(callback) {
    qzlVersion({ ...this.state.searchKey }, (res) => {
      this.setState({
        listResult: res.data,
      });
      callback && callback();
  ...
  handleSubmit() {
    this.props.form.validateFields((error, value) => {
      ...
      qzlVersionDetail({ ...value, id: this.state.detailId, action: 'edit' }, () => {
  ...
  deleteButton = (id) => {
    qzlVersionDetail({ id: id, action: 'delete' }, () => {
  ...
  renderColumn() {
    return [
      ...
      },
      {
        ...
        key: 'version_url',
        render: text => <a href={`${qzlOss}${text}`} target="__blank">下载</a>,
      },
      {
        title: '需要更新的版本号',
  ...
  render() {
    ...
    const uploadApp = {
      accept: '.apk',
      name: 'file',
      action: `${qzlUrl}/admin-base/upload`,
      onChange(info) {
        console.log(info);
