...
export default class app extends Component {
  ...
  // 获取活动列表数据
  getData(callback) {
    dogsVersionAd({ ...this.state.searchKey }, (res) => {
      this.setState({
        listResult: res.data,
      });
      callback && callback();
  ...
  handleSubmit() {
    this.props.form.validateFields((error, value) => {
      ...
      dogsVersionAdDetail({ ...value, action: 'add' }, () => {
  ...
  handleChange(id, name) {
    dogsVersionAdDetail({ version_id: id, app_name: name, action: 'change' }, () => {
      message.success('操作成功');
      this.getData();
    }, (res) => {
      message.warning(res.msg)
    ...
    return [
      ...
      },
      {
        ...
        render: (text, record) => (
          <span>
            <span>
              <a onClick={() => this.handleChange(record.version_number, record.source_name)}>{text === '1' ? '开启' : '关闭'}</a>
            </span>
          </span>
        ),
      },
