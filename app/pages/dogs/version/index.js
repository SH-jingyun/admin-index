...
export default class app extends Component {
  ...
  // 获取活动列表数据
  getData(callback) {
    dogsVersion({ ...this.state.searchKey }, (res) => {
      this.setState({
        listResult: res.data,
      });
      callback && callback();
  ...
  handleInfo(id) {
    dogsVersionDetail({ version_id: id }, (res) => {
      this.setState({
        detail: res.data,
        showDetail: true,
        detailId: id,
  ...
  handleSubmit() {
    this.props.form.validateFields((error, value) => {
      ...
      dogsVersionDetail({ ...value, version_id: this.state.detailId, action: 'edit' }, () => {
  ...
  deleteButton = (id) => {
    dogsVersionDetail({ version_id: id, action: 'delete' }, () => {
  ...
  renderColumn() {
    return [
      ...
      },
      {
        ...
        key: 'version_url',
        render: text => (text === '' ? '' : <a href={`${text}`} target="__blank">下载</a>),
      },
      {
        title: '需要更新的版本号',
            ...
            <Form layout="horizontal">
              <FormItem {...formItemLayout} label="版本号" hasFeedback>
                {getFieldDecorator('version_number', {
                  initialValue: this.state.detail.version_number || '',
