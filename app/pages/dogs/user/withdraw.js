...
export default class app extends Component {
  ...
  // 获取活动列表数据
  getData(callback) {
    dogsWithdraw({ ...this.state.searchKey }, (res) => {
      this.setState({
        listResult: res.data,
      });
      callback && callback();
  ...
  handleSuccess(id) {
    dogsWithdrawAction({ withdraw_id: id, action: 'success' }, (res) => {
      this.setState({}, () => {
        this.getData();
      });
    });
  ...
  handleFailed() {
    this.props.form.validateFields((error, value) => {
      ...
      dogsWithdrawAction({ ...value, withdraw_id: this.state.withdraw_id, action: 'failed' }, (res) => {
        this.setState({
          showReason: false,
          withdraw_id: 0,
        }, () => {
  ...
  renderColumn() {
    return [
      ...
      {
        title: 'Api返回信息',
        dataIndex: 'api_return_message',
        key: 'api_return_message',
      },
      {
        title: '提现申请时间',
